...
class FosterAEnv(gym.Env):
    ...
    def __init__(self):
        ...
        if self.race == 'major':
            self.reunification_rate =  np.random.beta(10,4)
        else:
            self.reunification_rate = np.random.beta(1,4)
        ## TODO: Sample more features using generative model. 
    def evaluate(self,race,age,policy,n_sample):
        reward_trajectory = []
        for i in range(n_sample):
            ...
            while self.status == 'in':
                action = policy[(self.race,self.age)]
                c,a,race,r,s=self.step(action)
                print(c,a,race,r,s)
                sum_reward += r
